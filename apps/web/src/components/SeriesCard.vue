...
import type { SeriesListItem } from '../api/client'
...
<template>
  <RouterLink :to="`/series/${series.publicId}`" class="card-link">
    <article class="media-card">
      <div class="media-card-cover series-card-cover">
        <div class="series-card-placeholder">
          <svg width="48" height="48" fill="currentColor" viewBox="0 0 24 24" opacity="0.35">
      ...
      <div class="media-card-body">
        <span class="media-card-title">{{ series.name }}</span>
        <span class="media-card-meta">{{ series.albumCount }} 张专辑</span>
      </div>
    </article>
  </RouterLink>
</template>
